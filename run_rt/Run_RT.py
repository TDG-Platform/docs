...
def main():
    ...
    command = vupt_exe + " -f " + full_filename
    ...
    if(parsed_parms.has_key('outputshpfilename')):
        ...
        out_shp_file_full = os.path.join(gbd_input_path, out_shp_file) 
        if(not os.path.exists(gbd_input_path)):
    ...
    if(transfer_support and app_status == 0):
        print 'Transferring data from ' + input_dir + ' to ' + outdir + ' ...'
        copytree(input_dir, outdir)
