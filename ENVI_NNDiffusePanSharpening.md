...
```python
	...
	pansharpTask.inputs.input_low_resolution_raster_format = '{"sensor type": "IKONOS"}'
	pansharpTask.inputs.input_high_resolution_raster_format = '{"sensor type": "IKONOS"}'
...
----------------- | -------------------------------------------------------
IKONOS   |   task.input_raster_format = '{"sensor type": "IKONOS"}'
Landsat-8   |     task.input_raster_format = '{"sensor type": "Landsat OLI"}'
Sentinel     |     task.input_raster_format = '{"sensor type": "SENTINEL-2"}'
...
ignore_validate  | False  | None    | number |  Set this property to a value of 1 to run the task, even if validation of properties fails. This is an advanced option for users who want to first set all task properties before validating whether they meet the required criteria. This property is not set by default, which means that an exception will occur if any property does not meet the required criteria for successful execution of the task.
